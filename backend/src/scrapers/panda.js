/**
 * Panda scraper (panda.com.sa)
 * Major KSA supermarket chain with online store
 */
...
logger.info('Panda scraper initialized', { 
...
async function parseResults(page) {
  ...
  try {
    // Panda product structure - try multiple selector strategies
    results = await page.evaluate(() => {
      // Try multiple selectors for Panda's product cards
      let productCards = Array.from(document.querySelectorAll('.product-item, .product-card, .product-tile, [class*="ProductCard"], [class*="product-card"], [data-product-id]'));
      
      // If no results, try alternative selectors
      if (productCards.length === 0) {
        productCards = Array.from(document.querySelectorAll('div[class*="Product"], article, [class*="item"], [class*="card"], [class*="tile"]'));
      }
      ...
      productCards.slice(0, 20).forEach(card => {
        try {
          ...
          // Find product URL
          let urlEl = card.querySelector('a[href*="/product"], a[href*="/p/"], .product-link, h2 a, h3 a, a[href*="panda.com"]');
          ...
          let priceEl = card.querySelector('.price, .product-price, [class*="price"], .amount, [class*="Price"], [data-price], [class*="amount"]');
          if (!priceEl) {
          ...
          let imageEl = card.querySelector('img[src*="panda"], img.product-image, img[class*="product"], img[data-src*="panda"], img');
          let imageUrl = null;
          if (imageEl) {
    ...
    logger.debug('Panda selector failed', { error: e.message });
    return [];
  }
  
  logger.debug('Panda scraper found items', { 
    totalFound: results.totalFound, 
    parsed: results.items.length 
  });
  
  // Normalize results - be more lenient with price requirement
  const normalized = results.items.filter(r => r.product_name && r.product_name.length > 3).slice(0, MAX_RESULTS).map(r => {
  ...
  const validated = validateResults(normalized, 'panda');
  
  logger.debug('Panda validation', { 
...
async function search(query) {
  const searchUrl = `https://www.panda.com.sa/en/search?q=${encodeURIComponent(query)}`;
  ...
  try {
    ...
    logger.error('Panda scraper failed', { 
      query, 
      error: error.message,
...
async function performSearch(query, searchUrl) {
  let browser, page;
  try {
    logger.debug('Starting Panda scraper', { query, url: searchUrl });
    ...
    logger.debug('Navigating to Panda search page', { query });
    ...
    try {
      ...
      if (navError.message && (
        ...
        logger.warn('Panda scraper: Domain not found, skipping scraper', { 
          query, 
          url: searchUrl,
    ...
    try {
      await page.waitForSelector('.product-item, .product-card, [class*="product"], [data-product-id]', { timeout: 20000 });
    } catch (e) {
      try {
        await page.waitForSelector('div[class*="Product"], article, [class*="item"]', { timeout: 15000 });
      } catch (e2) {
        logger.debug('Panda results selector not found, continuing...');
      }
    }
    ...
    logger.info('Panda scraper completed successfully', { query, resultCount: results.length });
    return results;
    ...
    if (error.message && (
      ...
      logger.warn('Panda scraper: Domain resolution failed, skipping scraper', { 
        query, 
        url: searchUrl,
        error: error.message 
      });
    ...
    await safeClosePage(page, 'panda');
    await safeCloseBrowser(browser, 'panda');
  }
}
